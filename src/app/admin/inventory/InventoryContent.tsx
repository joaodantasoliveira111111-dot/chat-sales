...
export function InventoryContent({
  ...
  const handleDelete = async () => {
    ...
    try {
      const supabase = createClient()
      await supabase.from('inventory_items').update({ status: 'archived' }).eq('id', deleteItem.id).eq('user_id', userId)
      ...
      toast.warning('Item excluído', 6000, {
        label: 'Desfazer',
        onClick: async () => {
          const { error: undoError } = await supabase.from('inventory_items').update({ status: 'available' }).eq('id', deletedItem.id).eq('user_id', userId)
          if (!undoError) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('inventory_items').update({ status: 'archived' }).in('id', ids).eq('user_id', userId)
      if (error) throw error
